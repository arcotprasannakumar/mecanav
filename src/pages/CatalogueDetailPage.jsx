...
function CatalogueDetailPage() {
  ...
  const unavailableMessage = useMemo(() => {
    ...
    return "The legacy catalogue detail route exists, but the matching PDF is not available in the current project assets.";
  }, [catalogue]);
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <aside className="space-y-6">
            <div className="rounded-[20px] border border-white/10 bg-white/5 p-5">
              <img
                src={catalogue.cardImage}
                alt={catalogue.title}
                className="w-full rounded-[16px] object-cover"
              />
              <h1 className="mt-5 text-2xl font-semibold text-white">{catalogue.title}</h1>
              <p className="mt-3 text-sm leading-7 text-white/70">{catalogue.summary}</p>
              {catalogue.overview ? (
                <p className="mt-3 text-sm leading-7 text-white/60">{catalogue.overview}</p>
              ) : (
                <p className="mt-3 text-sm leading-7 text-white/55">
                  The legacy catalogue detail pages do not include descriptive body copy beyond
                  the embedded preview, so this route keeps the focus on preview and download.
                </p>
              )}
            </div>

            <div className="rounded-[20px] border border-white/10 bg-white/5 p-5">
              <h2 className="text-lg font-semibold text-white">Preview & Download</h2>
              <p className="mt-3 text-sm leading-7 text-white/70">
                This follows the original catalogue flow: preview the flipbook, then unlock the
                PDF download through a short form gate.
              </p>
            ...
            <div className="rounded-[20px] border border-white/10 bg-white/5 p-5">
              ...
              <p className="mt-3 text-sm leading-7 text-white/70">{catalogue.legacySource}</p>
              <p className="mt-3 text-sm leading-7 text-white/55">
                Migration status: {catalogue.migrationStatus}
              </p>
            </div>
          </aside>
        </div>
